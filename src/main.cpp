...
static void logWifiStatus(bool force)
{
    ...
    lastWifiStatus = st;
    lastWifiLogMs = now;
    Serial.printf("kesp: wifi status=%d %s sta_ip=%s rssi=%ld target_ssid=%s\n",
                  (int)st,
                  wifiStatusName(st),
...
void setup()
{
    ...
    Serial.printf("kesp: wifi begin sta-only ssid=%s port=%u\n", WIFI_SSID, TCP_PORT);
...
void loop()
{
    ...
    if (now - lastMs >= 1000) {
        ...
        Serial.printf("kesp: alive=%lu sta_ip=%s status=%d spi=%d rx=%lu B/s qmax=%lu\n",

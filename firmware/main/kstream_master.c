...
static void transport_task(void *arg)
{
    ...
    for (;;) {
        ...
        for (unsigned i = 0; i < 3u; ++i) {
            ...
            size_t space = xStreamBufferSpacesAvailable(s_buffers->uplink);
            size_t queued = xStreamBufferBytesAvailable(s_buffers->uplink);
            if (wanted != 0u && space < wanted && queued != 0u)
                break;
...
{
    s_buffers = buffers;
    if (xTaskCreate(transport_task, "kstream_spi", 2048, NULL,
...
{
    TaskHandle_t task = s_transport_task;
    s_transport_task = NULL;
    __sync_synchronize();
    if (task && task != xTaskGetCurrentTaskHandle())
        vTaskDelete(task);
    ESP_ERROR_CHECK(gpio_set_intr_type(KSTREAM_READY_GPIO,
                                       GPIO_INTR_DISABLE));
    ESP_ERROR_CHECK(gpio_set_level(KSTREAM_CS_GPIO, 1u));
    ESP_ERROR_CHECK(gpio_set_level(KSTREAM_MASTER_PHASE_GPIO, 1u));
}
